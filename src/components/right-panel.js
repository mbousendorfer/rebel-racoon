...
import { isSidebarCollapsed, setSidebarCollapsed, isAutoCollapsed } from "./sidebar.js?v=288";
import {
...
import { open as openAddSourceModal } from "./add-source-modal.js?v=78";
import { open as openRenameModal } from "./rename-modal.js?v=3";
import { getConnectedConnectors } from "../connectors-store.js?v=41";
import { getSessionById } from "../sessions-store.js?v=21";
import { getContextById, getBrandKitGaps } from "../contexts-store.js?v=54";
import { quickGenerateUrl } from "../image-studio.js?v=96";
import { askConnector } from "../connector-ask.js?v=21";
import { renderConnectorLogo } from "../connectors-view.js?v=23";
import { open as openConnectorsModal } from "./connectors-modal.js?v=24";
import { addMention as addComposerMention } from "../composer-mentions.js?v=43";
import { iconFor } from "../file-kinds.js?v=21";
import { getIdeas, removeIdeasForSources } from "../library.js?v=70";
...
import { open as openScheduleModal } from "./schedule-modal.js?v=71";
import { open as openImageStudio } from "./image-studio-v2/index.js?v=99";
    ...
    if (postClipEditBtn) {
      ...
      openVideoClipsModal(source, {
        ...
        onSaveClips: (srcId, nextClips) => {
          ...
          if (!edited) return;
          import("../posts-store.js?v=50").then(({ updatePostClip }) => {
    ...
    if (sourceClipsBtn) {
      ...
      openVideoClipsModal(src, {
        ...
        onUseClips: (selectedClips, source) => {
          import("../screens/session.js?v=542").then(({ startClipDraftFlow }) => {
            startClipDraftFlow(
              sid,
...
function onSectionSave(network) {
  ...
  const count = snapshot.length;
  const draftWord = count === 1 ? "draft" : "drafts";
  Promise.all([import("./save-folder-modal.js?v=23"), import("../folders-store.js?v=18")]).then(
...
function useIdea(ideaId) {
  ...
  const sid = activeSessionId();
  if (!sid) return;
  import("../screens/session.js?v=542").then(({ askAngleQuestion }) => {
