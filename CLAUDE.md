...
src/
  ...
  modules/image-studio/ — the Image Generator (flag `sexySquirrel`): its own routes, stores, mocks, styles
  ...
  components/             — each exports init() (injects DOM once) + render/open()
    ...
    video-clips-modal.js, schedule-modal.js (+ schedule-modal-legacy.js behind `newScheduleModal`),
...
| `schedule-presets-store.js` | **global** one posting-rhythm preset per Playbook (rhythm · time of day · skipped days) — never on the fiche | `getPreset`, `savePreset`, `subscribe`                                                                                                                                                                                 |
