...
The reader sees **six states at one level** — To review · Trending · Updated · Already used · For later · Ignored — each a `.ap-status` pill with its own tone, glyph and word, and each one row in the Filters panel. `topicStates(topic)` derives that flat list from the four fields; `TOPIC_STATES` in `topics-catalog.js` is the single declaration. **That is presentation, not a merge** — and the separation is exactly what makes it work: a Topic can be **Already used and Trending** at once and both chips show. Collapsing the fields would let one fact hide another, let a re-scan overwrite the reader's own answer, and break the wire contract (`AC-TRK-6`) that reports the three independently.

A signal **never overrides the state filter**: an ignored Topic that starts trending stays hidden. Which is why the filter is **"untick a state to hide what carries it"** — a Topic shows only when every state it carries is ticked — and never an OR over ticked rows. As a collapsible "trending" section it had to override the filter, which made the filter lie. The cost, stated: a state can be hidden but not isolated.
...
⚠️ `.ap-segmented-control` was ported here for `/topics`' two-view switch and has been **deleted** — it was the wrong component (the product uses TABS for that shape, and only one list was ever on screen). Then the `.ap-tabs` that replaced it went too: `later` is one of the six states in the Filters panel now, so a second control that could disagree with the filter no longer exists. `git log -S renderTabs` has the tabbed version, `git log -S ap-segmented-control` the one before it. Don't re-port either without a genuine 2–4 co-visible-views case.
