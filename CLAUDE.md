...
**Per-session** stores: `library`, `posts-store`, `assistant`, `composer-mentions`. Everything else is global — the catalogs (`contexts`, `connectors`, `folders`, `sessions`, `schedule`, `top-posts`, `topics`) legitimately so, since they pre-exist any chat. The odd one out is **`sources-stream`**: pipeline content that is global anyway, plus the global ideas pool `library.js` keeps beside its per-session lists. That's a known gap, not the model — what a user brings into a chat belongs to that chat ([`docs/reference/CONCEPTS.md`](docs/reference/CONCEPTS.md) §3). `library.js` subscribes to sources-stream and re-emits per-session so any session's content surfaces repaint when a source lands. **No localStorage persistence of app state** — only `archie-user-mode`, the feature-flag keys, sidebar collapse state, and the single-use `sessionStorage` handoff keys.
