...
| `/home`              | `home.js`              | The account **HOME** (flag `playbookWorkspace`): a hero (prompt + Playbook picker + Send) over two tabs — Playbooks (the catalogue) and Chats (every chat, all brands). Account scope, no rail. Flag OFF the route bounces to `/` |
