...
## 8. Audit final (2026-09-25, après l'étape 7)

### Design System

| Contrôle                                            | Résultat                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Couleurs en dur dans le CSS du module               | **0**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Valeurs en px dans le CSS du module                 | Seulement les filets 1–2 px, le motif `sr-only` et 5 breakpoints (`768 / 1024 / 1200`) — le DS n'a pas de token pour ces trois cas                                                                                                                                                                                                                                                                                                                                                                            |
| Tokens utilisés mais inexistants (échec silencieux) | **0** (`--imst-stage-ratio` et `--imst-swatch` sont posés en inline par le module)                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Icônes `ap-icon-*` inexistantes                     | **0** (trois corrigées en route : `-text` → `-alt-text`, `-undo` → `-rotate-left`, `-eye` n'a jamais été utilisé)                                                                                                                                                                                                                                                                                                                                                                                             |
| Classes `.ap-*` redéclarées dans le module          | **0** — le module compose en `.imst-*`                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Règles maison                                       | Encre grey-80 → grey-100 ; filets grey-20 ; bleu = interactif ; hover de carte = bordure bleue, sans ombre ni barre d'accent ; pas de majuscules ; boutons auto-width ; chaque score nommé en texte (« Brand check score 82 / 100 », « Predicted performance 64 / 100 »)                                                                                                                                                                                                                                      |
| Composants DS utilisés                              | `.ap-button`, `.ap-icon-button`, `.ap-tabs`, `.ap-select` (+ groupes, options à case), `.ap-action-dropdown`, `.ap-dialog`, `.ap-snackbar`, `.ap-infobox`, `.ap-card`, `.ap-radio-card`, `.ap-checkbox-container`, `.ap-toggle-container`, `.ap-input-group`, `.ap-textarea-field`, `.ap-form-field`, `.ap-form-message`, `.ap-tag`, `.ap-status`, `.ap-counter`, `.ap-table`, `.ap-loader` ; ports de `ds-patches.css` : `.ap-filter-chip`, `.ap-slider`, `.ap-dropzone`, `.app-ring`, `.app-modal-backdrop` |
| Aucun nouveau composant DS                          | Compositions signalées en §3 : pastille couleur, état vide, liste de calques, stage, maquettes réseau, overlays (safe zones, heatmap)                                                                                                                                                                                                                                                                                                                                                                         |

**Écarts assumés, à connaître :**

- Le contenu généré (`render/`, `config/demo-data.js`, les mocks) contient des couleurs en dur : c'est du contenu, pas du chrome (exception du brief). Les maquettes réseau et la heatmap sont en tokens DS.
- `--app-*` (largeur `--app-content`, rayons, `--app-shadow-popover-lg`, z-index) — autorisé en §6.1.
- Le stage de l'éditeur est limité à `70vh` de haut ; l'interligne du texte sur l'image (1.2) est fixé pour que l'écran et l'export PNG coïncident.
- Les pastilles de couleur prennent leur tooltip du `[data-tooltip]` global d'Archie (dépendance douce : sans lui, l'`aria-label` reste).

### Ce qui a changé hors du module

Calculé commit par commit sur mes commits (une autre session pousse aussi sur `main`), `?v=` neutralisé :
