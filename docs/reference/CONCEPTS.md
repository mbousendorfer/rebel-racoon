...
Le **kit de marque** (flag `sexySquirrel`, 2026-09-25) entre par la même porte : la **version** de chaque logo (couleur / blanc / noir / icône), le **rôle** de chaque couleur (primaire / secondaire / accent seulement / fond / texte), les **moods** de l'imagerie, les **mots à éviter** (famille Voice), et les **règles visuelles** — do / don't, taille minimale et zone de protection du logo, interdiction de le déformer, paires de couleurs interdites. Tous disent à quoi la marque ressemble et restent vrais seuls ; aucun ne lance rien. Le générateur d'images (FEATURES §18) les **lit** ; il n'a **plus** ses propres marques — un premier jet lui en donnait, avec des sous-marques héritées, et ça a été retourné le jour même : la marque, c'est le Playbook. Les déclinaisons régionales suivent la règle de granularité ci-dessous (dupliquer, puis diverger), pas un héritage.
...
| **Création** (Image Generator) | Un visuel généré, ses déclinaisons, calques et légende — plus les styles, produits et campagnes du générateur (flag `sexySquirrel`) | Du contenu produit, **clé par Playbook** (`brandId` = id du Context) dans le stockage du module. Le générateur **lit** la marque sur le Playbook (§1, kit de marque) et ne l'écrit jamais ; il n'a pas de marques à lui. Voir [`FEATURES.md`](FEATURES.md) §18. |
