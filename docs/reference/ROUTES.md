...
| `/home`              | `home.js`              | La **home du compte** (flag `playbookWorkspace`) : un héro (prompt + picker de Playbook + Send) au-dessus de deux onglets — **Playbooks** (le catalogue) et **Chats** (tous les chats, toutes marques). Account scope, pas de rail. Flag OFF, la route rebondit sur `/`.                                                                                                                                                        |
...
Exemples en service : `#/session/:id?tab=…&focusIdea=…`, `#/topics?pb=…` (flag OFF), et `#/home?tab=playbooks|chats` — les deux listes de la home, dans l'URL pour que le Back du navigateur passe de l'une à l'autre ; une valeur inconnue retombe sur `playbooks`.
