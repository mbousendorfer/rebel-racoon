...
| `/image-generator/*` | `modules/image-studio/index.js` | **Image Generator** (flag `sexySquirrel`, default OFF ; deep-link périmé → `/`) — le module déclare ses sous-routes (`ROUTES`), `app.js` les enregistre en boucle. `/image-generator` = Generate (le hub), `/image-generator/history`, `/image-generator/catalog`, `/image-generator/styles` (+ `/styles/new` enregistré **avant** `/styles/:id`, que « new » satisferait), `/image-generator/editor/:creationId`. Query : `?creation=` (rouvrir un lot), `?style=` / `?product=` (présélection) ; sur l'éditeur `?format=` (l'onglet actif) et `?adapt=1` (ouvre « Adapt everywhere »). Pas de route de marques : la marque est le Playbook (`/playbook/:id`). Route de workspace (rail complète). Voir [`FEATURES.md`](FEATURES.md) §18. |
