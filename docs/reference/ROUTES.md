...
| `/contexts`          | `contexts.js`          | Library **Playbooks** : cards (DO/DON'T, brief, color tag) + edit en side panel. Sous `playbookWorkspace` : **hors du shell** (`body.account-scope`, voir ci-dessous), atteinte par le footer du switcher.                                                                                         |
| `/playbook/:id`      | `playbook.js`          | Page détail d'un Playbook. Topbar back → `/contexts`. Sous `playbookWorkspace` la fiche de la marque **active** est une route in-workspace (rail conservée, pas de back, titre « Playbook ») ; celle d'une autre marque est en account scope, back → `/contexts`.                                  |
...
| **Account** (`body.account-scope`) | ce qui parle des **autres** marques : `/contexts`, `/playbook/<autre>`                         | **pas de rail**, une colonne, topbar réduite au retour (« ‹ Back to <marque> » depuis le catalogue, « ‹ Back to all playbooks » depuis une fiche) |
