...
| `/image-generator/*` | `modules/image-studio/index.js` | **Image Generator** (flag `sexySquirrel`, default OFF ; deep-link périmé → `/`) — module autonome, qui déclare ses propres sous-routes (`ROUTES`) ; `app.js` les enregistre en boucle. `/image-generator` = Generate (le hub), `/campaigns`, `/brands`. Route de workspace (rail complète). Voir [`FEATURES.md`](FEATURES.md) §18.                                                                                              |
