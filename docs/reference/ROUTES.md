...
| `/image-generator/*` | `modules/image-studio/index.js` | **Image Generator** (flag `sexySquirrel`, default OFF ; deep-link périmé → `/`) — le module déclare ses sous-routes (`ROUTES`), `app.js` les enregistre en boucle. `/image-generator` = Generate (le hub), `/image-generator/campaigns`. Pas de route de marques : la marque est le Playbook (`/playbook/:id`). Route de workspace (rail complète). Voir [`FEATURES.md`](FEATURES.md) §18.                                      |
