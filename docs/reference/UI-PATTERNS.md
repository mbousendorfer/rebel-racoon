...
- **Un composant qui vit dans DEUX sortes de liste sort deux cadres, et sa règle de base n'en porte aucun.** `topic-card` rend une **ligne** dans le feed (aucun cadre propre, un filet en bas, le cadre appartient à la colonne — une liste continue à la façon d'un lecteur de mail) et une **carte** dans la dialog du picker (une liste courte où chaque item est un contrôle). Toute déclaration de cadre — `background`, `border`, `border-radius`, et les états de survol qui les teintent — vit sur la variante, jamais sur la base : une `border` partagée est exactement ce qui a fait fuir un cadre de carte dans le lecteur. Le test qui le prouve est d'ouvrir le picker après avoir touché au feed.
- **Sur une ligne, l'état est un REMPLISSAGE ; sur une carte, une bordure.** Une ligne n'a pas de cadre à teinter. Survol en `electric-blue-05`, sélection un cran plus ferme en `electric-blue-10` — et ⛔️ jamais de bordure gauche colorée, qui est la tentation de tout master–detail.
