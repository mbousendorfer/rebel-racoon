...
| [`contexts-store.js`](../../src/contexts-store.js)       | Playbooks (Contexts en code). Porte aussi l'appartenance (`ownerId` / `scope` / `sharedWith` / `history`, flag `playbookSharing`) — mais **ne filtre jamais** : les droits vivent dans [`playbook-access.js`](../../src/playbook-access.js), voir le § ci-dessous.                                                                                                                                                                                                                                            | `getContexts`, `getContextById`, `getDefaultContext`, `addContext`, `updateContext`, `duplicateContext`, `deleteContext`, `appendHistory`, `subscribe`                                                           |
...
**Le feed est scopé, donc ses compteurs le sont.** Le compteur de la ligne de nav est le nombre de Topics **à revoir** du feed du Playbook par défaut, pas une somme sur tout le compte : la surface qu'il envoie ouvrir n'en montre qu'un. Quel Playbook, ça vient de `playbookForNewWork()` ([`active-playbook.js`](../../src/active-playbook.js)) : le Playbook **actif** sous le flag `playbookWorkspace`, le Playbook par défaut sinon — la même résolution que l'écran vers lequel le compteur envoie, pour qu'ils ne puissent jamais nommer deux marques différentes. Ce que le fork avait raté, ce n'est pas le scope global mais le scope **caché** : son `active-playbook.js` se faisait écrire par un select posé dans une barre de filtres, sans switcher visible. Flag OFF, `?pb=` dit la même chose et s'arrête à son écran ; flag ON, la rail porte le scope et `?pb=` n'est plus lu.
