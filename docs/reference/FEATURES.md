...
- **Contrôle Playbook** : sur chat neuf = dropdown sélectionnable (+ « Create a playbook ») ; sur chat actif = indicateur statique. Sous `playbookWorkspace` il est **toujours** l'indicateur statique : la question a été posée dans la rail avant que le chat existe (§14).
...
- **Switcher de Playbook** (flag `playbookWorkspace`) : entre le head et la nav — le wordmark, c'est le produit ; ce contrôle, c'est la marque à laquelle appartient tout ce qui est en dessous. Un `.ap-select` DS (dot couleur + label inline « Playbook » + nom + chevron), les Playbooks visibles en options, un footer **Manage playbooks** / **Create a playbook**. Rail replié : les initiales de la marque dans un `.ap-avatar square size-24` — cliquer **rouvre la rail** (un scope illisible ne se change pas). Aucun Playbook (new-alt) : la ligne devient l'offre d'en créer un. Un switch depuis un chat emmène sur le chat le plus récent de la marque choisie, ou sur un chat neuf ; ailleurs la page se repeint sur place (`subscribe` d'`active-playbook`).
