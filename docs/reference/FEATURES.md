...
**Deux rangées de page, pas trois — la première appartient au shell.** Le motif d'en-tête maison en compte trois (titre, toolbar, tabs), mais `topbar.js` imprime déjà le nom de la route : l'écran dessine donc **UNE seule rangée** : les onglets à gauche, le cluster de portée (select + cog) et Filters à droite. ⚠️ C'étaient deux rangées empilées, et avec de vrais articles dans le volet ce n'était pas un coût abstrait — **chaque pixel au-dessus du lecteur est un pixel de prose à faire défiler**. Mesuré : 109px d'en-tête → **49**, et le lecteur démarre à 133 au lieu de 193. La rangée porte le filet, pas le nav des onglets (un filet qui s'arrête au dernier onglet souligne une demi-rangée), et `.ap-tabs` étant `width: 100%` il faut le shrink-wrap (`width: max-content`) sinon les contrôles repassent à la ligne — la recette de `.rpanel-drafts__statustabs`, scopée par l'hôte comme les trois autres hôtes d'onglets de l'app. La toolbar porte **deux groupes** : le cluster de portée — le select en label inline `Playbook │ …` collé à son cog de réglages, qui configure précisément l'écoute de ce Playbook — puis, après un écart plus large, Filters libellé avec son compteur inline. ⚠️ Le cog a été épinglé à l'extrême droite pendant deux commits, sur une lecture du motif produit qui en avait manqué la condition ; voir [`UI-PATTERNS.md`](UI-PATTERNS.md) § En-tête d'écran. Voir [`UI-PATTERNS.md`](UI-PATTERNS.md) § En-tête d'écran.
