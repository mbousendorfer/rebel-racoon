...
⚠️ **Les onglets `.ap-tabs` sont supprimés** — et avant eux un segmented control, qui était le mauvais composant (le produit emploie des tabs partout où cette forme apparaît, et on ne voyait **qu'une** liste à la fois). Le port du segmented control dans `ds-patches.css` était déjà parti ; `git log -S renderTabs` rend la version à onglets, `git log -S ap-segmented-control` celle d'avant. L'axe `kind` (To review / For later) que les onglets portaient est redevenu un contrôle : le **radio « Topics »** du panneau de filtres (voir Les filtres), un seul à la fois mais dans l'unique contrôle, donc rien ne peut le contredire.
...
- **Topics** — un **RADIO** (`.ap-radio-container`) sur `kind` : « Read one kind at a time ». **To review** (la voie active, `ready`) / **For later** (la voie parkée, `later`), un seul à la fois — un Topic a exactement un `kind`. C'est l'ancien axe à deux segments (`AC-SEG`), de retour en radio **dans** le panneau plutôt qu'en onglets au-dessus de la liste. Défaut : To review.
...
- **⚠️ L'ouverture des selects est dans `view`** (`markedOpen` / `sourcesOpen`), pas laissée à `<details>` : cocher déclenche `change`, qui repeint le panneau, donc un `<details>` nativement ouvert se refermerait à chaque clic. Un seul ouvert à la fois ; le clic du `summary` est intercepté (`preventDefault`). Escape ferme le select ouvert **seul** (la chose la plus intérieure), puis le panneau, puis le volet.
