...
**LES DEUX COLONNES PARTAGENT LEUR LIGNE HAUTE, et rien ne la calcule.** Elles sont enfants du même flex `.topics-view__split` et aucune ne se décale, donc la ligne est une conséquence du DOM. Le compteur de la liste est l'en-tête **fixe** de sa colonne — hors scrollport, comme `pane-head` est hors de `pane-body` — et son inset du haut (`sm + xxxs + 1px`) pose sa ligne de texte sur celle du volet : le bord de la carte (1px), le `padding-top` de `pane-head` (`sm`), et la moitié du mou de `.topic-article__provenance`, dont le `min-height` de 24 centre une caption de 16 (`xxxs`). Mesuré : 142..156 des deux côtés.

⚠️ **C'était un inset arithmétique, et il avait déjà dérivé.** Le volet portait `margin-top: var(--topic-group-offset)` — la boîte du séparateur d'âge, 20px — pour que **son bord de carte rencontre celui de la première carte**. Puis `.topics-view__list-head` (28px) et le `gap: sm` du scrollport (16px) se sont installés au-dessus des groupes : la vraie distance est passée à 64px, l'offset est resté à 20, et le bord haut du volet s'est retrouvé **44px au-dessus de la première carte et au niveau de rien**. Un offset dérivé d'un seul élément ne survit pas à l'insertion d'un second au-dessus de lui, et rien ne prévient. Le coût, énoncé : le bord du volet ne rencontre plus un bord de carte — il rencontre l'en-tête de l'autre colonne, qui est la ligne qu'un lecteur parcourt réellement et la seule qui ne puisse pas devenir fausse.

⚠️ **Le compteur portait le type d'une accroche de carte** — `--sys-text-style-h4-*` en encre `default`, exactement `.topic-card__headline` deux rangées plus bas. Le nombre qui décrit la liste concurrençait le contenu qu'il décrit, et la ligne la plus forte de la colonne était celle qui portait le moins. Il passe en caption encre `light` : de l'appareil. Il reste **non filtré** (il décrit le feed, pas la sélection) et **disparaît pendant le scan** — un total posé à côté de cinq cartes-fantômes est un nombre sur du contenu qui n'est pas dessiné. L'alignement s'en fiche, ce qui est précisément ce que la ligne partagée achète.
