...
- **La liste** : le batch se lit comme un calendrier éditorial, dans **une seule liste encadrée** (filet grey-20, rayon carte), avec des filets entre les lignes. Les lignes sont **triées par date**, et chaque ligne a **deux zones qui ne se mélangent jamais** :
  - **Quand** : une cellule de 216px sur fond grey-05, séparée par un filet. Elle contient une **vignette de date** (jour de la semaine · grand chiffre · mois, encre grise sur blanc, jamais bleue), **l'heure** en h3 avec un crayon (`.ap-icon-button` nu) qui ouvre le sélecteur natif via `showPicker()` sur un `datetime-local` caché, puis au plus deux notes courtes en caption. **Une date posée dans la meilleure fenêtre de son réseau porte ma sparkle en pastille sur le coin de la vignette** (disque blanc, filet grey-20, `ap-icon-sparkles ap-icon-xs` orange). Son tooltip et son `aria-label` donnent le détail : _« LinkedIn's best window: Tue–Thu, 9 AM »_. Les autres raisons restent en texte sous l'heure, avec la sparkle : _« Best hour »_ (hors des meilleurs jours du réseau) et _« Best morning hour »_. Pour une date posée à la main, le texte est _« Set by you · Reset »_. La seconde, **ce qui est déjà prévu ce jour-là**, est un lien : _« +2 posts that day »_ (icône calendrier grise), ou en orange _« Too close: 30 min »_ (icône warning, tooltip _« LinkedIn post at 9:00 AM »_) quand un post du **même réseau** tombe à moins de 2 h. Le lien déplie **l'agenda du jour** sous la ligne, sur toute sa largeur et sur fond grey-05 : chaque post à son heure, avec la glyphe du réseau et sa 1re ligne. Le draft y est marqué `This draft` (status blue), le post en conflit `30 min apart` (status orange), un autre draft du batch `This batch` (status grey). Un seul agenda ouvert à la fois. Le réseau est déjà nommé par le profile tag voisin ; le détail de la raison est dans le tooltip (_« LinkedIn's best window: Tue–Thu, 9 AM »_).
  - **Quoi** : le profile tag et 2 lignes du texte, puis **la miniature du média** (carré de 56px recadré), puis le ✕ (_Leave this draft out_). Une image affiche l'image. Un carrousel affiche sa 1re slide avec un badge _« 1/3 »_. Un clip vidéo (`clipRef`, sans poster) affiche la trame teintée de sa `hue` (comme `post-card`), une icône play et sa durée. Sans média, pas de miniature. Les drafts de démo `post-acme-1/3/4` portent une image, un carrousel et une image, pour que la miniature se voie.
  ...
  - En dessous de 600px, « Quand » passe au-dessus de « Quoi », séparé par un filet horizontal.
