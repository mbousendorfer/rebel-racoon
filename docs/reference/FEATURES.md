...
Les **starter cards** viennent d'un renderer partagé ([`components/starter-card.js`](../../src/components/starter-card.js)) que le héro du chat et la home du compte (§12) appellent tous deux — donc leur mise en page se règle en un seul endroit. Le sous-titre a une **mesure plafond** (46ch) et non une bride : il était capé à 22ch, un quart de la largeur de la carte, si bien qu'une phrase de 70 caractères se cassait en quatre lignes et laissait la moitié droite vide. Le watermark qu'elle évitait est un lavis à 10 % que le texte recouvre de toute façon. Le plancher de la carte (140px, pas 158) est ce que la rangée mesure quand tous les sous-titres sont courts — la grille étire les trois à la plus haute de toute façon.
