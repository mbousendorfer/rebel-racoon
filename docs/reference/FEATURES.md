...
- **La liste** : le batch se lit comme un calendrier éditorial, dans **une seule liste encadrée** (filet grey-20, rayon carte), avec des filets entre les lignes. Les lignes sont **triées par date**. Chaque ligne se lit **QUOI → QUAND** : le post à gauche, sa date à droite. Toutes les cellules prennent la hauteur de la ligne, mais leur contenu **part d'une même ligne de tête** : la miniature, le profile tag, la vignette de date et l'heure sont sur la première ligne de 24px ; les notes s'empilent dessous. ⚠️ Le centrage vertical a été essayé puis abandonné (_« ça flotte fort, rien n'est aligné »_) : des cellules au contenu différent ont des milieux différents, donc rien ne tombait à la même hauteur d'une ligne à l'autre. Le post est à gauche parce que c'est l'objet qu'on reconnaît avant de décider quand il part. Les dates à droite forment une colonne qu'on relit d'un coup d'œil, juste au-dessus du primaire. La chronologie reste portée par le tri.
  - **Quand** (à droite) : une cellule de 192px (la vignette, l'heure et la note la plus large, rien de plus) sur fond grey-05, séparée par un filet à gauche, qui ne porte **que de l'information**. Elle contient une **vignette de date** (jour de la semaine · grand chiffre · mois, encre grise sur blanc, jamais bleue), **l'heure** en h3, puis au plus deux notes courtes en caption. La vignette et l'heure sont cliquables : elles ouvrent le sélecteur natif via `showPicker()` sur un `datetime-local` caché sous l'heure, et l'heure passe en bleu au survol.
  ...
  - **Les actions de la ligne, regroupées et au survol** : le crayon (_Change date or time_) et le ✕ (_Leave this draft out_, en multi seulement) forment une petite barre (`.ap-icon-button` nus, sur fond blanc, sans ombre). Elle flotte en haut à droite du post, contre la colonne date, et n'apparaît qu'au survol ou au focus de la ligne ; sur écran tactile, elle est toujours visible. Elle ne coûte donc aucune largeur au repos. ⚠️ Deux étapes avant : le crayon collé à l'heure et le ✕ au bout de la ligne, puis les deux dans une colonne d'actions permanente, qui laissait trop de vide dans le bloc date.
