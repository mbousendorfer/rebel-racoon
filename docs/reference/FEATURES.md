...
- **Relevance et Why now sont une liste de définition** (`<dl>`), pas deux boîtes : les labels partagent une colonne en `max-content`, donc les deux phrases démarrent sur le même x et la paire se lit comme deux **rangées**. Icône + label en caption-bold encre claire à gauche, phrase en body encre par défaut à droite, un filet dessous. ~70px au lieu de ~130, et elles ne concurrencent plus l'analyse qu'elles introduisent.
