...
- **Un Topic `later` N'A PAS de version détaillée, par définition.** Le slot de prose porte donc ce fait à la place de la prose : un empty state (« Not enough to write a detailed version yet ») sur le sol gris. Clé sur le **kind**, pas sur l'absence d'article — le kind _est_ l'affirmation, et rendre trois paragraphes sous un onglet « Topics for later » disait le contraire de l'onglet. Relevance, Why now et les Contributing posts restent : ce qui manque est l'analyse, pas le contexte. Aucun bouton dedans — le primaire de l'en-tête EST l'action que cette copie nomme.
...
- **Relevance et Why now sont DEUX BLOCS DE LA MÊME FORME** : une icône, un label, une phrase, sur le sol gris (`--app-surface-subtle`), rayon de carte, sans bordure ni ombre. `ap-icon-target` pour Relevance (à qui c'est destiné), `ap-icon-clock` pour Why now (le moment). Une grille à trois cellules et non une rangée flex, pour que la phrase s'aligne sous son **label** et non sous l'icône : une colonne d'icône que la prose dépasse ensuite, c'est la forme d'un item de liste, et ce n'en est pas un.
  - **La hiérarchie descend le label**, elle ne monte pas la phrase : caption-bold en encre claire pour le label, body en encre par défaut pour la phrase.
  - **Écart `xxs` entre les deux blocs** contre `xs` de l'article vers la prose : l'espace dedans doit être plus petit que l'espace autour.
  - **Grayscale, rien de plus.** Le label nomme le fait, le glyphe lui donne une silhouette ; dépenser une couleur d'accent sur du métadonnée ne laisserait rien pour les actions. Et **pas** `.topic-badge` pour le glyphe, aussi tentant que ce soit : ce pip signifie « quelle source d'écoute a produit ça » et l'en-tête de l'article en porte un 40px au-dessus. Le même carré ne peut pas dire deux choses sur une surface.
  - ⚠️ **C'étaient deux espèces différentes.** Relevance était un paragraphe avec un « Relevance: » gras en ligne, Why now une **infobox du DS** — donc l'un se lisait comme de la prose résiduelle et l'autre comme une alerte système, à 8px d'écart. L'infobox était doublement fausse : c'est le composant d'un message que l'**app** vous adresse, alors que ce sont des attributs du Topic ; et un aplat teinté surclassait l'analyse qu'il introduit. Le gras foncé du label concurrençait par ailleurs son propre contenu.
...
- **La TRACE du Topic, en dernier et REPLIÉE.** `Topic history N` sur l'accordéon du DS. En dernier parce que l'ordre de lecture est faits → analyse → preuve → ce qui est arrivé au Topic : la trace est méta, elle ne fait pas partie de l'argument. ⚠️ La capture de la branche d'origine la mettait _avant_ les Contributing posts ; je m'en écarte pour cette raison. Repliée parce que **38 des 50 Topics qui en ont une n'ont qu'UNE entrée** — une section ouverte en permanence dépenserait un titre et une rangée pour une ligne de provenance.
