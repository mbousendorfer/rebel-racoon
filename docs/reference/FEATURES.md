...
- **La liste** : le batch se lit comme un calendrier éditorial, dans **une seule liste encadrée** (filet grey-20, rayon carte), avec des filets entre les lignes. Les lignes sont **triées par date**, et chaque ligne a **deux zones qui ne se mélangent jamais** :
  - **Quand** : une cellule de 200px sur fond grey-05, séparée par un filet. Elle contient une **vignette de date** (jour de la semaine · grand chiffre · mois, encre grise sur blanc, jamais bleue), **l'heure** en h3 avec un crayon (`.ap-icon-button` nu) qui ouvre le sélecteur natif via `showPicker()` sur un `datetime-local` caché, puis au plus deux notes courtes en caption. La première dit **pourquoi cette heure**, avec la sparkle orange (_« Best window »_ / _« Best hour »_ / _« Best morning hour »_), ou bien _« Set by you · Reset »_. La seconde dit si le jour est chargé (_« +1 post that day »_). Le réseau est déjà nommé par le profile tag voisin ; le détail est dans le tooltip : _« LinkedIn's best window: Tue–Thu, 9 AM »_, _« Also that day: 9:00 AM LinkedIn »_.
