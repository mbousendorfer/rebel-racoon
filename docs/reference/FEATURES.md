...
- **LE PLAFOND DES POSTS DÉPEND DU `kind`, parce que leur RÔLE en dépend.** Sur un `ready`, ils **étayent** un argument que le lecteur vient de lire : les deux premiers s'affichent, le reste passe derrière « N more posts » (se déclenche sur 9 des 34 `ready` ; la moyenne est de 2,0, donc la plupart ne plafonnent jamais). Sur un `later`, il n'y a pas d'angle draftable et les posts **SONT** la matière — plafonner viderait le panneau, et 6 des 18 n'en ont déjà aucun.
  - Le compteur de section annonce toujours le **total**, donc le panneau ne cache jamais _combien_ de preuve existe : il la diffère. Le déclencheur disparaît une fois déplié — « 4 more posts » au-dessus de six posts visibles se contredit.
  - Le dépli est une checkbox voisine, comme la trace, et pour la même raison : ce renderer est pur et a trois hôtes.
...
- **Relevance et Why now vivent dans l'EN-TÊTE**, pas dans le corps. Ce sont les critères de **triage** — à qui c'est destiné, et pourquoi maintenant — donc ils appartiennent à l'identité et aux verbes qui agissent dessus, pas à l'analyse qui défile. Dans le volet l'en-tête est fixe, donc ils restent aussi en vue pendant qu'on lit.
  - Une **liste de définition** (`<dl>`) et non deux boîtes : les labels partagent une colonne en `max-content`, donc les deux phrases démarrent sur le même x. `align-items: start` — pas le `stretch` par défaut, qui étirait le `<dt>` sur la hauteur de la rangée et laissait son `inline-flex` centrer le label au milieu d'une phrase de quatre lignes. Le label porte la `line-height` du **body**, donc sa boîte vaut exactement une ligne du texte d'à côté et les deux premières lignes partagent une ligne de base (mesuré : delta 0).
  - ⚠️ **Conséquence assumée : l'en-tête FIXE varie en hauteur selon le Topic** — 125px sans eux (16% du volet), ~257 avec les deux (28%). C'est le prix de garder les critères de triage en vue pendant que la prose défile, et ça ne tombe que sur la minorité qui les porte (relevance 9 sur 52, why now 13). Si le saut entre Topics coûte plus que la persistance ne rapporte, le correctif est de rendre ce bloc au scroller — **jamais** de tronquer les phrases.
