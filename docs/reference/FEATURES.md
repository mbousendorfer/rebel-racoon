...
- **Résumé du choix** : sur une surface grey-05, un disque blanc portant la sparkle orange (le loader Archie pendant le calcul) et **une phrase dont les variables sont en gras** — _« **Every weekday** from **Sat, Sep 26**, at **each network's best time**. »_ (un seul draft : _« From **Sat, Sep 26**, at **LinkedIn's best time**. »_). Le bouton ghost bleu **Adjust** déplie les réglages, **fermés par défaut** :
  ...
  - **Enregistrer le rythme par Playbook** : la dernière rangée d'Adjust. Un bouton `stroked grey` _« Save as Acme · Q2 marketing's rhythm »_ (ou _« Update … »_ si le rythme a divergé), avec _« I'll start from it every time you schedule for this Playbook. »_. Une fois enregistré : ✓ _« This is Acme · Q2 marketing's rhythm — … »_. La modale **s'ouvre sur le preset** du Playbook du chat (`playbookId` passé par `right-panel` depuis `session.contextId`), et la phrase l'annonce : _« **Acme · Q2 marketing's rhythm** — Twice a week from … »_. Le preset retient le rythme, le moment de la journée et les jours exclus, **jamais la date de départ** (toujours demain). Seul le **propriétaire** du Playbook peut l'enregistrer (`canEdit`) ; les autres lisent _« Only the owner of … can save its posting rhythm. »_. Stockage : [`schedule-presets-store.js`](../../src/schedule-presets-store.js), clé par Playbook, en mémoire comme tous les stores (perdu au rechargement). Ce n'est **pas** un champ de la fiche : voir [`CONCEPTS.md`](CONCEPTS.md) §1.
...
- **La liste** : le batch se lit comme un calendrier éditorial, dans **une seule liste encadrée** (filet grey-20, rayon carte), avec des filets entre les lignes. Les lignes sont **triées par date**, et chaque ligne a **deux zones qui ne se mélangent jamais** :
  - **Quand** : une cellule de 216px sur fond grey-05, séparée par un filet. **La vignette de date et l'heure forment un seul bouton** qui ouvre le sélecteur natif (`showPicker()` sur un `datetime-local` caché). Au repos, elles se lisent comme de la donnée (encre grise, pas de crayon) ; au survol ou au focus, l'heure passe en bleu et un crayon apparaît. Sur écran tactile, le crayon est toujours visible. **Pourquoi cette heure** est dans le tooltip (_« LinkedIn's best window: Tue–Thu, 9 AM »_) : la phrase du haut dit déjà _« at each network's best time »_ une fois pour tout le batch. Sous l'heure, **seulement s'il y a quelque chose à dire** : _« Set by you · Reset »_, et **ce qui est déjà prévu ce jour-là**. Ce second lien affiche _« +2 posts that day »_, ou en orange _« Too close: 30 min »_ (icône warning, tooltip _« LinkedIn post at 9:00 AM »_) quand un post du **même réseau** tombe à moins de 2 h. Il déplie **l'agenda du jour** sous la ligne, sur toute sa largeur et sur fond grey-05 : chaque post à son heure, avec la glyphe du réseau et sa 1re ligne. Le draft y est marqué `This draft` (status blue), le post en conflit `30 min apart` (status orange), un autre draft du batch `This batch` (status grey). Un seul agenda ouvert à la fois.
  - **Quoi** : 2 lignes du texte, **la miniature du média** (carré de 56px recadré), puis le ✕ (_Leave this draft out_), visible au survol ou au focus de la ligne et toujours sur écran tactile. Une image affiche l'image. Un carrousel affiche sa 1re slide avec un badge _« 1/3 »_. Un clip vidéo (`clipRef`, sans poster) affiche la trame teintée de sa `hue` (comme `post-card`), une icône play et sa durée. Sans média, pas de miniature. **Le profile tag n'apparaît que si le batch mélange plusieurs réseaux.** Sinon le profil est nommé une fois, dans le sous-titre : _« On Northwind Studio Co.'s LinkedIn. I picked a time for each draft… »_. Les drafts de démo `post-acme-1/3/4` portent une image, un carrousel et une image, pour que la miniature se voie.
  - ⚠️ **Au repos, une ligne = la date, l'heure, le texte, la miniature.** Tout le reste (raison, crayon, ✕, profil répété) a été retiré de la ligne après un retour _« forte complexité globale »_ : ne pas le remettre au premier plan. Deux versions précédentes ont aussi été rejetées. D'abord, l'heure, le crayon, le profil, le texte et les métadonnées sur une même ligne : le draft ne se lisait plus comme un objet. Ensuite, des vignettes sur un rail vertical, avec des cartes de draft détachées à droite : _« ça flotte beaucoup trop »_, rien ne tenait la date à son draft. C'est le cadre commun et les filets qui tiennent les lignes.
...
- **Footer** : à gauche, ce que le batch représente (_« **4 posts over 8 days** · Sep 29 – Oct 6 »_, ou _« **Tue, Sep 29** at 9:00 AM »_), sans la disclosure, retirée ; à droite, Cancel · **Schedule N posts** (désactivé seulement pendant le calcul et l'envoi). Succès → toast _« N post(s) scheduled »_.
