...
- **La preuve est sur son propre FOND**, un panneau en creux (`--app-surface-subtle` sur `--app-radius-card`, `sm` de padding — la recette d'encart qui existe déjà dans `posts.css`), pas derrière un filet. L'analyse est la prose d'Archie ; ces posts sont ceux de quelqu'un d'autre, cités : un autre genre de chose, et un filet entre les deux ne disait que « section suivante ». Le filet est parti avec, le changement de fond dessinant déjà la frontière — les garder tous les deux signale le même bord deux fois. Les cartes gardent leur fond blanc explicite, donc elles restent des cartes sur ce gris au lieu de se dissoudre en une dalle, et rien n'imbrique d'élévation : le panneau comme les cartes sont plats.
