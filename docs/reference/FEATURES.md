...
### Modal Schedule ([`schedule-modal.js`](../../src/components/schedule-modal.js)) — flag `newScheduleModal` (« New SM », **ON** par défaut ; OFF rouvre l'ancienne modale, [`schedule-modal-legacy.js`](../../src/components/schedule-modal-legacy.js), §14) — 720px, une colonne, le résultat d'abord
...
- **Résumé du choix** : sur une surface grey-05, un disque blanc portant la sparkle orange (le loader Archie pendant le calcul) et **une phrase dont les variables sont en gras** — _« **Every weekday** from **Sat, Sep 26**, at **each network's best time**. »_ (un seul draft : _« From **Sat, Sep 26**, at **LinkedIn's best time**. »_). Le bouton ghost bleu **Adjust** déplie les réglages, **fermés par défaut** :
  ...
  - **Skip** : sept `.ap-filter-chip` à bascule (`aria-pressed`, pressé = jour exclu), lundi d'abord, précédés du libellé « Skip ». Cocher les sept est refusé. Des chips plutôt que des checkboxes, parce qu'ils prennent deux fois moins de largeur et laissent l'action du preset sur la même rangée.
  - Chaque changement **relance le calcul**.
  - **Enregistrer le rythme par Playbook** : au bout de la rangée _Skip_, **sans séparateur ni légende**. Trois états :
    1. **Aucun rythme enregistré** : un `ghost blue` _« Save for this Playbook »_, dont le tooltip nomme le Playbook (_« Acme · Q2 marketing starts from this rhythm every time you schedule for it. »_).
    2. **Les réglages sont le rythme du Playbook** : ✓ _« This Playbook's rhythm »_.
    3. **Un rythme existe et les réglages en divergent** : _« Not this Playbook's rhythm · **Restore** · **Update** »_ (tooltip : le rythme enregistré en clair). _Restore_ remet le rythme du Playbook en gardant la date de départ ; _Update_ l'écrase. Changer un réglage pour un batch ne réécrit donc jamais en silence le rythme de la marque.

    Pour un non-propriétaire, l'état 1 devient _« Only the owner can save it »_, et l'état 3 n'offre que _Restore_. ⚠️ Une première version en faisait une section à part (séparateur, bouton `stroked` pleine étiquette, ligne d'explication) : _« le save est très proéminent »_. Elle ne gérait pas non plus le Playbook qui a déjà un rythme. La modale **s'ouvre sur le preset** du Playbook du chat (`playbookId` passé par `right-panel` depuis `session.contextId`), et la phrase l'annonce : _« **Acme · Q2 marketing's rhythm** — Twice a week from … »_. Le preset retient le rythme, le moment de la journée et les jours exclus, **jamais la date de départ** (toujours demain). Stockage : [`schedule-presets-store.js`](../../src/schedule-presets-store.js), clé par Playbook, en mémoire comme tous les stores (perdu au rechargement). Ce n'est **pas** un champ de la fiche : voir [`CONCEPTS.md`](CONCEPTS.md) §1.

- **Moteur** : parcourt les jours qui matchent le rythme et saute : les jours exclus, les jours qui portent déjà un post **sur un des réseaux du batch**, et ceux des drafts épinglés. Un post Facebook ne bloque donc pas un jour pour LinkedIn : ce jour reste éligible et sa ligne dit ce qu'il contient. Ensuite, une date par draft à la meilleure heure du réseau (`PER_NETWORK_OPTIMAL`), biaisée par Time of day. Un draft seul prend le premier jour libre parmi les meilleurs jours de son réseau. Débordement empilé sur le dernier jour, une heure d'écart.
- **La liste** : le batch se lit comme un calendrier éditorial, dans **une seule liste encadrée** (filet grey-20, rayon carte), avec des filets entre les lignes. Les lignes sont **triées par date**. Chaque ligne se lit **QUOI → QUAND** : le post à gauche, sa date à droite. Toutes les cellules prennent la hauteur de la ligne, mais leur contenu **part d'une même ligne de tête** : la miniature, le profile tag, la vignette de date et l'heure sont sur la première ligne de 24px ; les notes s'empilent dessous. ⚠️ Le centrage vertical a été essayé puis abandonné (_« ça flotte fort, rien n'est aligné »_) : des cellules au contenu différent ont des milieux différents, donc rien ne tombait à la même hauteur d'une ligne à l'autre. Le post est à gauche parce que c'est l'objet qu'on reconnaît avant de décider quand il part. Les dates à droite forment une colonne qu'on relit d'un coup d'œil, juste au-dessus du primaire. La chronologie reste portée par le tri.
  - **Quand** (à droite) : une cellule de 216px (la vignette, l'heure et son crayon, et la note la plus large sur **une** ligne ; une note ne passe jamais à la ligne) sur fond grey-05, séparée par un filet à gauche, qui ne porte **que de l'information**. Elle contient une **vignette de date** (jour de la semaine · grand chiffre · mois, encre grise sur blanc, jamais bleue), **l'heure** en h3, puis au plus deux notes courtes en caption. La vignette et l'heure sont cliquables : elles ouvrent le sélecteur natif via `showPicker()` sur un `datetime-local` caché sous l'heure, et l'heure passe en bleu au survol.
    - **Toute date que j'ai choisie porte ma sparkle en pastille sur le coin de sa vignette** (disque blanc, filet grey-20, `ap-icon-sparkles ap-icon-xs` orange). Son tooltip et son `aria-label` disent _« LinkedIn's best time »_ (_« … in the morning »_ avec un moment de la journée). **Un seul concept, le _best time_ du réseau, et un seul traitement.** ⚠️ Une version distinguait _« Best window »_ (en pastille) de _« Best hour »_ / _« Best morning hour »_ (en texte), pour un seul et même concept : _« y'a pas de best hour ou best day, c'est best time tout simplement »_. Le tooltip ne cite pas non plus les meilleurs jours du réseau, parce que sur une date prise hors de ces jours par le rythme, il se contredisait.
    - Une date posée à la main n'a pas de pastille ; sous son heure, elle porte _« Set by you · Reset »_.
    - **Ce qui est déjà prévu ce jour-là** est un lien : _« +2 posts that day »_ (icône calendrier grise), ou en orange _« 30 min apart »_ / _« Same time »_ (icône warning, tooltip _« LinkedIn post at 9:00 AM »_) quand un post du **même réseau** tombe à moins de 2 h. Le lien ouvre **l'agenda du jour dans un popover** ancré au lien : une surface DS `.ap-action-dropdown` de 380px, rendue à la racine du dialog pour ne pas être rognée par la liste, et positionnée en JS contre la boîte du dialog (qui est centré par transform). Il liste chaque post à son heure, avec la glyphe du réseau et sa 1re ligne. Le draft y est marqué `This draft` (status blue), le post en conflit `30 min apart` (status orange), un autre draft du batch `This batch` (status grey). Il se ferme par un clic ailleurs, par Échap (avant le dialog, grâce à un listener en capture) et au scroll du corps du dialog. Un seul à la fois. ⚠️ Il se dépliait sous la ligne et poussait toute la liste : _« un peu perturbant »_.
  - **Quoi** (à gauche) : **la miniature du média en tête** (carré de 56px recadré), puis le profile tag et 2 lignes du texte. Une image affiche l'image. Un carrousel affiche sa 1re slide avec un badge _« 1/3 »_. Un clip vidéo (`clipRef`, sans poster) affiche la trame teintée de sa `hue` (comme `post-card`), une icône play et sa durée. **Un post sans média garde la colonne** : une vignette neutre grey-05 avec l'icône `ap-icon-quote`, pour que le texte de chaque ligne commence au même x. Les drafts de démo `post-acme-1/3/4` portent une image, un carrousel et une image.
  - **Chaque action dans le bloc de l'objet sur lequel elle agit**, au survol ou au focus de la ligne (toujours visible sur écran tactile). Le **crayon** (_Change date or time_) est dans la cellule date, à côté de l'heure ; sa place est réservée, donc rien ne bouge quand il apparaît. La vignette et l'heure sont elles-mêmes cliquables. Le **✕** (_Leave this draft out_, en multi seulement) est dans le coin haut-droit du post, sur fond blanc, sans ombre. ⚠️ Trois étapes avant : le crayon collé à l'heure et le ✕ au bout de la ligne, puis les deux dans une colonne d'actions permanente (trop de vide dans le bloc date), puis les deux dans le coin du post, cellule date à 192px : _« le change date & time il doit pas être dans le bloc du draft ! »_, et des notes passaient sur deux lignes.
  - ⚠️ La date était à gauche, sur une cellule teintée, avec la miniature au milieu de la ligne et absente des posts sans média : les lignes _« flottaient un peu »_. D'où l'inversion QUOI → QUAND, la miniature en tête et toujours présente, et l'alignement sur une ligne de tête commune.
  - ⚠️ **Ne pas réduire la complexité en retirant de l'information.** Une passe a retiré de la ligne le profil (le réseau), la raison, le crayon et la disclosure, pour ne garder au repos que la date, l'heure, le texte et la miniature. Elle a été annulée : _« t'as supprimé plein d'informations importantes, comme le nom du network »_. Le réseau, la raison et le crayon restent sur chaque ligne. Deux versions précédentes ont aussi été rejetées. D'abord, l'heure, le crayon, le profil, le texte et les métadonnées sur une même ligne : le draft ne se lisait plus comme un objet. Ensuite, des vignettes sur un rail vertical, avec des cartes de draft détachées à droite : _« ça flotte beaucoup trop »_, rien ne tenait la date à son draft. C'est le cadre commun et les filets qui tiennent les lignes.
  - En dessous de 600px, « Quand » passe sous « Quoi », séparé par un filet horizontal.
- **Hauteur constante, ligne ET dialog** : le sous-titre du calcul (_« I'm finding the best time for each draft… »_) et celui du résultat (_« I picked the best time for each draft… »_) ne diffèrent que d'un verbe, pour se couper pareil. Le dialog garde donc sa hauteur quand les dates arrivent (mesuré : 658px en multi, 385px en single, pendant et après). ⚠️ _« … Change any of them. »_ faisait passer le résultat sur deux lignes : +24px au moment où les dates arrivaient.
- **Hauteur de ligne constante** : une ligne garde la même hauteur pendant le calcul, une fois sa date arrivée et une fois épinglée avec un conflit. La vignette a une hauteur fixe (`--tile-h: 66px`) dans ses deux états (shimmer / date), et les notes (au plus deux) tiennent dans cette hauteur. L'agenda du jour flotte en popover et n'agrandit rien.
...
- **Éditer une date l'épingle** : _« Set by you · Reset »_, et la ligne rejoint sa place dans la frise. Les réglages ne l'écrasent plus ; Reset la rend au moteur.
...
| `sexySquirrel`           | Sexy Squirrel — AI Image Generator | **OFF** | Tout le module **Image Generator** (§18) : la route `/image-generator/*` (un deep-link rebondit sur `/`) et sa ligne de nav. Flag OFF, le module n'écrit rien en storage.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `newScheduleModal` | New SM (schedule modal) | **ON** | **Quelle modale Schedule s'ouvre depuis le panneau Drafts.** ON (défaut) = la refonte décrite au §5 : dates proposées à l'ouverture, phrase + _Adjust_, rythme par Playbook, liste QUOI → QUAND. OFF = l'ancienne modale (cartes Optimal / Custom, chips de cadence, stratégie en texte libre, _Compute best times_ qui débloque Schedule, calendrier mensuel), conservée telle quelle dans [`schedule-modal-legacy.js`](../../src/components/schedule-modal-legacy.js) + `schedule-modal-legacy.css`, classes renommées `schedule-modal-legacy` pour ne pas entrer en collision. Le choix se fait en un seul endroit, `openScheduleModal()` dans `right-panel.js`. Une fois la comparaison terminée, les deux fichiers legacy seront à supprimer. |
...
- **Generate** (le hub) : la carte de marque, puis **quatre idées de campagne** (mock `ideaService` : deux des trois prochains événements du calendrier pour le secteur, un produit du catalogue, un angle de marque — chacune avec titre, angle, style et formats conseillés, et un aperçu) ; « More ideas » en propose d'autres. Un clic remplit le brief ; on peut toujours écrire le sien. La **barre de prompt** : le brief, un texte optionnel pour l'image, et quatre pickers DS (`.ap-select` à label intégré) — **Style** (ceux du Playbook en tête), **Product**, **Formats** (multi, par réseau), **Text** (« Built into the image » est désactivé, avec la raison, quand le style ne le supporte pas ; changer pour un tel style repasse en calque et le dit). **Generate** (⌘↵) : quatre variations en 2–4 s, ou « Generation failed — try again ». Chaque variation : **Edit**, régénérer, « More like this » (un nouveau lot), favori, **télécharger en PNG** (rastérisation dans le navigateur, calques dessinés au canvas). Chaque génération est une création enregistrée (l'historique) ; un brief venu d'une idée ouvre ou rejoint la campagne de même titre. `?creation=` rouvre un lot, `?style=` présélectionne un style.
- **Éditeur** (`/image-generator/editor/:creationId`, ouvert par « Edit » sur une variation) : **calques** à gauche (image, texte, logo, forme, élément de la bibliothèque — afficher / masquer, verrouiller, ordre), **stage** au centre (glisser pour déplacer, coin pour redimensionner — logo et images gardent leurs proportions ; flèches ±1 %, Maj ±5 %, Suppr, ⌘Z), **propriétés** à droite : pour un texte, contenu, police et couleur limitées à la marque (« Use any font / colour » les déverrouille, et « Save … to the Playbook » garde un ajustement — la seule écriture du générateur sur un Playbook, explicite), taille, alignement, bandeau ; pour le logo, sa version et sa taille comparée au minimum du Playbook ; pour l'image, **nouveau fond / nouveau sujet** seul et la luminosité. **Text ideas** : trois accroches (dont une tirée des exemples de voix du Playbook) et trois CTA. **Edit in words** (mock `editService`) : « logo in white, bottom right », « bigger text », « add a band », « lighter background »… ce qui n'est pas compris est dit, jamais simulé. **Apply the brand** remet polices, couleurs (avec contraste) et version de logo dans la marque, et dit ce qui a changé. Download PNG exporte l'état exact du stage.
- **Formats** : l'éditeur a un onglet par format de la création (le **master** et ses déclinaisons, chacune avec ses propres calques — une retouche sur un format ne touche pas les autres). **Adapt everywhere** (dans l'éditeur, ou l'icône d'une variation sur le hub) choisit parmi les 11 formats des 4 réseaux et **recompose** chacun depuis le master : le fond est redessiné au nouveau ratio avec les mêmes seeds (jamais un recadrage), le texte et le logo vont à la place que prévoit le gabarit du format (`render/layout.js` : square / portrait / tall / wide), un fond de forme derrière le texte le suit, et la taille du texte s'ajuste pour tenir dans son bloc. **Show safe zones** hachure ce que l'interface du réseau recouvre (stories : barre du haut, champ de réponse en bas). **Preview in context** montre chaque format dans une maquette de feed, de story ou de carte lien de son réseau (chrome en gris DS). **Download** : ce format, ou tous les formats adaptés (un PNG chacun).
- **Catalog** (`/image-generator/catalog`) : les produits du Playbook actif — ajout depuis la page produit (mock `productService` : nom, description et image tirés de l'URL) ou à la main (photo uploadée, gardée aussi en petit data URL pour pouvoir être dessinée DANS l'image générée), modifier, supprimer. Un produit choisi comme sujet est dessiné depuis sa propre image, donc reconnaissable d'un visuel à l'autre. **Product shoot** : trois scènes (studio, lifestyle, saisonnier), enregistrables sur le produit, « Use » ouvre le hub avec le produit et le style de la scène.
- **Post** (onglet de l'éditeur) : pour le réseau du format actif, une légende écrite dans la voix de la marque, avec le compteur de la limite du réseau (X 280, Instagram 2 200, LinkedIn 3 000, Facebook 63 206) et ce qui s'affiche avant « …more », des hashtags dans la fourchette qui marche sur ce réseau, **Copy caption** / **Copy hashtags**. La maquette de Preview in context reprend la légende.
- **Checks** (onglet de l'éditeur) : **Brand check** — score sur 100 et problèmes nommés, chacun avec **Fix** (et « Fix all ») : couleur hors palette, logo sous la taille minimale, boîte du logo hors de ses proportions (un logo n'est jamais étiré), texte au contraste insuffisant (mesuré sur les pixels réels derrière le texte), texte ou logo hors safe zone, texte qui déborde, mot à éviter, paire de couleurs interdite, zone de protection du logo. **Predicted performance** (mocké) — un score sur 100 et les facteurs qui le font, chacun nommé, + la **heatmap d'attention** en overlay.
- **Campaigns** : le tableau des campagnes (période, événement, nombre d'images) et l'**historique** des créations — rouvrir, supprimer (confirmé).
- **Styles** (`/image-generator/styles`) : les styles du Playbook actif en tête, puis les 22 presets par famille (filtre `.ap-filter-chip`), chaque vignette dessinée **dans la palette de la marque active**. Un preset s'ouvre en aperçu (trois sujets neutres) avec « Make it your own » / « Use this style ». Le **créateur** (`/styles/new`, `/styles/:id`) mélange jusqu'à 10 images de référence et 5 presets, un poids chacun (part affichée en %), un mode de fidélité (Essential / Style & composition), un prompt de style optionnel, et un **test sur trois sujets neutres** avant d'enregistrer. Un style est enregistré pour un Playbook et passe en tête de son sélecteur ; il sait écrire du texte dans l'image si son preset le plus lourd le sait.
- **Rendu mocké** ([`render/`](../../src/modules/image-studio/render/)) : un générateur SVG procédural par preset — clay doux et arrondi, infographie en barres sur grille, cinématique en letterbox — seedé et dans la palette de la marque. Un style personnalisé dessine avec son preset le plus lourd (Style & composition) ou tire un preset par seed selon les poids (Essential) ; les couleurs échantillonnées de ses images de référence teintent la palette selon leur poids.
