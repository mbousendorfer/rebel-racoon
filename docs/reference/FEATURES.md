...
- **La frise** : le batch se lit comme un calendrier éditorial. Les lignes sont **triées par date**, et chaque ligne a **deux zones qui ne se mélangent jamais** :
  - **Quand**, à gauche, sur un **rail** grey-20 : une **vignette de date** (jour de la semaine · grand chiffre · mois, encre grise sur blanc, jamais bleue), **l'heure** en h3 avec un crayon (`.ap-icon-button` nu) qui ouvre le sélecteur natif via `showPicker()` sur un `datetime-local` caché, puis au plus deux notes courtes en caption. La première dit **pourquoi cette heure**, avec la sparkle orange (_« Best LinkedIn window »_ / _« Best LinkedIn hour »_ / _« Best morning hour »_), ou bien _« Set by you · Reset »_. La seconde dit si le jour est chargé (_« +1 post that day »_). Le détail est dans le tooltip : _« LinkedIn's best window: Tue–Thu, 9 AM »_, _« Also that day: 9:00 AM LinkedIn »_.
  - **Quoi**, à droite : **le draft comme un bloc à lui** (filet grey-20, rayon carte, sans ombre ni hover, puisqu'il n'est pas cliquable) : le profile tag, 2 lignes du texte, et son unique action ✕ (_Leave this draft out_) dans son coin.
  - ⚠️ Une première version de la frise mêlait l'heure, le crayon, le profil, le texte et les métadonnées sur une même ligne : le draft ne se lisait plus comme un objet. Ne pas remettre ces éléments côte à côte.
  - En dessous de 600px, « Quand » passe au-dessus de « Quoi » et le rail disparaît.
