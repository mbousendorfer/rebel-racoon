...
- **Résumé du choix** : sur une surface grey-05, un disque blanc portant la sparkle orange (le loader Archie pendant le calcul) et **une phrase dont les variables sont en gras** — _« **Every weekday** from **Sat, Sep 26**, at **each network's best time**. »_ (un seul draft : _« From **Sat, Sep 26**, at **LinkedIn's best time**. »_). Le bouton ghost bleu **Adjust** déplie les réglages, **fermés par défaut** :
  ...
  - **Skip** : sept checkboxes DS en ligne, lundi d'abord, précédées du libellé « Skip » (cocher les sept est refusé).
  - Chaque changement **relance le calcul**.
  - **Enregistrer le rythme par Playbook** : au bout de la rangée _Skip_ (« Skip » + les sept jours à gauche, l'action à droite, **une seule rangée, sans séparateur ni légende**). Un bouton `ghost blue` _« Save for this Playbook »_ (ou _« Update for this Playbook »_ si le rythme a divergé), dont le tooltip nomme le Playbook : _« Acme · Q2 marketing will start from this rhythm every time you schedule for it. »_. Une fois enregistré : ✓ _« Saved for this Playbook »_. ⚠️ Une première version en faisait une section à part (séparateur, bouton `stroked` pleine étiquette, ligne d'explication) : _« le save est très proéminent »_. C'est le geste rare du panneau, il ne prend pas de hauteur. La modale **s'ouvre sur le preset** du Playbook du chat (`playbookId` passé par `right-panel` depuis `session.contextId`), et la phrase l'annonce : _« **Acme · Q2 marketing's rhythm** — Twice a week from … »_. Le preset retient le rythme, le moment de la journée et les jours exclus, **jamais la date de départ** (toujours demain). Seul le **propriétaire** du Playbook peut l'enregistrer (`canEdit`) ; les autres lisent _« Only the owner of … can save its posting rhythm. »_. Stockage : [`schedule-presets-store.js`](../../src/schedule-presets-store.js), clé par Playbook, en mémoire comme tous les stores (perdu au rechargement). Ce n'est **pas** un champ de la fiche : voir [`CONCEPTS.md`](CONCEPTS.md) §1.
...
- **La liste** : le batch se lit comme un calendrier éditorial, dans **une seule liste encadrée** (filet grey-20, rayon carte), avec des filets entre les lignes. Les lignes sont **triées par date**. Chaque ligne se lit **QUOI → QUAND** : le post à gauche, sa date à droite, puis le ✕ dans une colonne à lui. Toutes les cellules prennent la hauteur de la ligne, mais leur contenu **part d'une même ligne de tête** : la miniature, le profile tag, la vignette de date, l'heure et le ✕ sont sur la première ligne de 24px ; les notes s'empilent dessous. ⚠️ Le centrage vertical a été essayé puis abandonné (_« ça flotte fort, rien n'est aligné »_) : des cellules au contenu différent ont des milieux différents, donc rien ne tombait à la même hauteur d'une ligne à l'autre. Le post est à gauche parce que c'est l'objet qu'on reconnaît avant de décider quand il part. Les dates à droite forment une colonne qu'on relit d'un coup d'œil, juste au-dessus du primaire. La chronologie reste portée par le tri.
  - **Quand** (à droite) : une cellule de 200px sur fond grey-05, séparée par un filet à gauche, qui ne porte **que de l'information** (vignette, heure, notes).
  - **Les actions de la ligne sont regroupées** dans une colonne à droite, prolongeant le fond grey-05 : le crayon (_Change date or time_, qui ouvre le sélecteur natif) puis le ✕ (_Leave this draft out_, seulement en multi-drafts). Les deux sont des `.ap-icon-button` nus, calés sur la ligne de l'heure. ⚠️ Le crayon était collé à l'heure et le ✕ seul au bout de la ligne : deux actions de la même ligne à deux endroits. Elle contient une **vignette de date** (jour de la semaine · grand chiffre · mois, encre grise sur blanc, jamais bleue), **l'heure** en h3 (le crayon, dans la colonne d'actions, ouvre le sélecteur natif via `showPicker()` sur un `datetime-local` caché sous l'heure), puis au plus deux notes courtes en caption. **Toute date que j'ai choisie porte ma sparkle en pastille sur le coin de sa vignette** (disque blanc, filet grey-20, `ap-icon-sparkles ap-icon-xs` orange). Son tooltip et son `aria-label` disent _« LinkedIn's best time »_ (_« … in the morning »_ avec un moment de la journée). **Un seul concept, le _best time_ du réseau, et un seul traitement.** ⚠️ Une version distinguait _« Best window »_ (en pastille) de _« Best hour »_ / _« Best morning hour »_ (en texte), pour un seul et même concept : _« y'a pas de best hour ou best day, c'est best time tout simplement »_. Le tooltip ne cite pas non plus les meilleurs jours du réseau, parce que sur une date prise hors de ces jours par le rythme, il se contredisait. Une date posée à la main n'a pas de pastille ; sous son heure, elle porte _« Set by you · Reset »_. La seconde, **ce qui est déjà prévu ce jour-là**, est un lien : _« +2 posts that day »_ (icône calendrier grise), ou en orange _« Too close: 30 min »_ (icône warning, tooltip _« LinkedIn post at 9:00 AM »_) quand un post du **même réseau** tombe à moins de 2 h. Le lien déplie **l'agenda du jour** sous la ligne, sur toute sa largeur et sur fond grey-05 : chaque post à son heure, avec la glyphe du réseau et sa 1re ligne. Le draft y est marqué `This draft` (status blue), le post en conflit `30 min apart` (status orange), un autre draft du batch `This batch` (status grey). Un seul agenda ouvert à la fois. Le réseau est déjà nommé par le profile tag voisin ; le détail de la raison est dans le tooltip (_« LinkedIn's best window: Tue–Thu, 9 AM »_).
