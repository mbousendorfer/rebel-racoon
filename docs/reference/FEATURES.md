...
- **LA SECTION EST REPLIÉE, et son état de repos DÉPEND DU `kind`** — parce que le rôle des posts en dépend. Sur un `ready`, ils **étayent** un argument que le lecteur vient de finir : replié est le bon état de repos. Sur un `later`, il n'y a pas d'angle draftable et les posts **SONT** la matière — les fermer viderait le panneau, l'erreur que `8d8d0b4b` a déjà commise en masquant le vrai contenu d'un `later`. Donc `later` s'ouvre déplié. Un contrôle, deux états de repos.
  ...
  - Le compteur de section annonce toujours le **total**, donc replier ne cache jamais _combien_ de preuve existe : ça la diffère.
