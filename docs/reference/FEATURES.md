...
- **Le produit, c'est la génération** : on arrive sur **Generate** (la marque active, la barre de prompt, quatre variations), ensuite l'éditeur à calques, les déclinaisons par format et l'export PNG. **History** range ce qu'on a produit.
...
- **Generate** (le hub) : la carte de marque, puis la **barre de prompt** : le brief, un texte optionnel pour l'image, et quatre pickers DS — **Style** (ceux du Playbook en tête), **Product**, **Formats** (multi, par réseau), **Text** (« Built into the image » est désactivé, avec la raison, quand le style ne le supporte pas). **Generate** (⌘↵) : quatre variations en 2–4 s, ou « Generation failed — try again ». Chaque variation : **Edit**, régénérer, « More like this », favori, Adapt everywhere, **télécharger en PNG**. Chaque génération est une création enregistrée (l'historique). `?creation=` rouvre un lot, `?style=` / `?product=` présélectionnent. ⚠️ **Pas de campagnes ni d'idées de campagne** : les deux ont été supprimées le 2026-09-25 à la demande (entité, page, table, idées générées depuis le calendrier) — ne pas les reproposer sans demande.
...
- **History** (`/image-generator/history`) : toutes les créations du Playbook actif, la plus récente en tête — rouvrir, supprimer (confirmé), réinitialiser la démo.
