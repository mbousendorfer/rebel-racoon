...
**Sous le flag, l'onglet est une LISTE de cards horizontales** (`.contexts-view__list`), pas une grille de tuiles. Une ligne = deux lignes de texte (identité + brief coupé net à l'ellipse), donc une seule hauteur pour toute la liste ; et surtout trois cellules **fixes** — les compteurs (104px), la date (128px), le kebab (32px) — pour que les nombres se lisent en colonne du haut en bas. En `auto`, la grappe de droite démarrait à un x différent selon les droits de chaque fiche, ce qui annule le seul gain d'une liste. Ce qui varie (chip de voix, tags `Current` / propriété) vit dans la cellule flexible.
...
| **Identité**        | dot de scope + nom + ★ défaut + chip de voix + `Current`, puis le brief sur une ligne                                  | la seule cellule flexible                                                                            |
| **Compteurs**       | chats · audiences · competitors                                                                                        | 104px                                                                                                |
