...
- **`/welcome-alt`** (redirect) : ajoute `body.onboarding` (full-bleed, pas de sidebar/topbar), mint une session `welcome-alt-{ts}`, arme le handoff `pendingStartContextBuilder` (`flow:"alt"`, `prefilledUrl`), navigue dans la session. Chat 3 questions (URL → profil → documents optionnels ; +langue si `multilingualPlaybook`, **−profil si `skipConnectProfiles`** — voir §14). L'ordre des étapes et leur numérotation sont dérivés d'une seule liste (`altSteps()` dans `context-builder.js`), donc ajouter ou retirer une étape renumérote les autres tout seul.
...
### Feature flags ([`ff-catalog.js`](../../src/ff-catalog.js)) — les 7
...
| `skipConnectProfiles` | Skip connecting profiles at setup | **OFF** | **Où l'app réclame un compte social.** OFF (défaut) = la création de Playbook demande le profil qui publiera en question 2, et les ~40 comptes de démo sont connectés d'entrée. ON = cette question disparaît (création à 3 étapes, 4 avec `multilingualPlaybook`) et **rien n'est connecté au départ**, dans les deux user modes. La demande se déplace là où elle est utile : les flows de chat qui draftent POUR un compte — draft depuis une idée, draft depuis des clips, repurpose (inline + studio) — remplacent leur étape « choisis un compte » par une étape « connecte un compte » au même endroit, et reprennent là où ils en étaient une fois la modale confirmée ([`connect-profiles-flow.js`](../../src/connect-profiles-flow.js) + [`connect-account-modal.js`](../../src/components/connect-account-modal.js)). Les autres surfaces à profils (Clip Studio, Objectives, modale analyze-profiles) rendent simplement leur liste vide. |
