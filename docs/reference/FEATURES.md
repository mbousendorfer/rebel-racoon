...
- **Why now est l'INFOBOX DU DS**, pas une ligne maison : `.ap-infobox info` avec son icône et sa paire titre/message, la même anatomie que la notice de partage de `playbook.js`. ⚠️ C'était un paragraphe avec un `border-left` coloré de 2px, teinté par signal — autrement dit **l'infobox du DS réimplémentée à la main** : `.ap-infobox::before` EST une barre colorée pleine hauteur de 4px, et le composant apporte en plus le fond teinté, le rayon, le padding et l'icône. La tonalité est `info` quel que soit le signal : les tonalités du DS sont **sémantiques** (`warning` = prudence, `success` = confirmation) et ni Trending ni Updated n'est l'un des deux — peindre un pic en avertissement est la même erreur que peindre Ignore en rouge. **Conséquence assumée : l'article ne code plus le signal en couleur.** La carte le dit toujours en mots ; si le volet doit le dire aussi, la marque va dans l'en-tête à côté de la source, pas dans une tonalité qui la nommerait mal.
