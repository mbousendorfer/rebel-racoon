...
**Sous le flag, l'onglet est une LISTE de cards horizontales** (`.contexts-view__list`), pas une grille de tuiles. Une ligne = deux lignes de texte (identité + brief coupé net à l'ellipse), donc une seule hauteur pour toute la liste ; et **toutes les cellules sauf l'identité sont fixes** — owner (132px), access (20px), la date (128px), le kebab (32px) — pour qu'elles se lisent en colonne du haut en bas. En `auto`, la grappe de droite démarrait à un x différent selon les droits de chaque fiche, ce qui annule le seul gain d'une liste. Ce qui varie (le tag `Current`) vit dans la cellule flexible.

**Les colonnes disent d'un Playbook ce que Claude Design dit d'un design system** — c'est le même genre d'objet, l'identité depuis laquelle une machine écrit :

| Colonne             | Contenu                                                                                                                | Note                                                                                                 |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| **Vignette** (40px) | le **logo** de la marque quand l'analyse en a trouvé un (`brandLogo`), sinon ses initiales dans un `.ap-avatar square` | `object-fit: contain` sur fond blanc : le DS _couvre_ ses images, et un lockup couvert revient rogné |
| **Identité**        | dot de scope + nom + ★ défaut + `Current`, puis le brief sur une ligne                                                 | la seule cellule flexible                                                                            |
| **Owner**           | avatar + « You » / le nom du propriétaire                                                                              | **flag `playbookSharing` seulement** — sans lui, neuf lignes identiques                              |
| **Access**          | `ap-icon-pen` si je peux éditer, `ap-icon-eye-on` sinon ; le `title` porte la phrase (« Shared by Sam Rivera »)        | idem                                                                                                 |
| **Updated**         | l'horodatage                                                                                                           | 128px                                                                                                |
| **⋯**               | les quatre verbes                                                                                                      | 32px                                                                                                 |

Ce que Claude Design a et qu'un Playbook n'a pas : **Published** (rien à publier ici) et l'**étoile favori** — la nôtre est le badge « défaut », en lecture seule : le flag `playbookDefault` et son toggle ont été supprimés (§14), et ça ne revient pas. Le tag de propriété inline (« Shared with org ») a disparu de la ligne d'identité : ses deux faits sont devenus deux colonnes qui s'alignent.

⚠️ **Pas de ghost card en fin de liste** : `Create a Playbook` est déjà le primaire de la barre d'outils deux lignes plus haut. Sur une grille, la tuile fantôme remplit la dernière cellule d'une forme qui semblerait inachevée ; une liste n'a pas ce trou, donc c'était la même offre deux fois. La grille flag OFF la garde.

⚠️ **Trois choses que la tuile porte et que la ligne ne porte pas**, chacune retirée après avoir vu neuf lignes côte à côte : les **compteurs** (chats / audiences / competitors — trois nombres par ligne que personne ne lit pour choisir une marque, dans la colonne la plus large de la page) ; le **chip de voix** (une tuile a la place de caractériser, une ligne a un nom et, juste dessous, la phrase de la marque qui le dit mieux) ; et les **dots de palette** (la vignette et le dot portent déjà l'identité visuelle). La tuile garde les trois : une grille de neuf cartes se parcourt, cette liste se scanne, et scanner veut moins de colonnes.

Trois écarts de forme avec la tuile, chacun pour une raison : la couleur de la marque passe du **bandeau haut à un point** (une barre d'accent sur le bord d'une ligne de liste a déjà été recalée ici, et le point est la marque que la rail et l'onglet Chats utilisent déjà) ; les verbes passent de la **barre au survol à un kebab** `⋯` (`installMoreMenu`, mêmes hooks `data-contexts-*` — une barre dont la largeur dépend des droits déplace toute la grappe, et une commande révélée au survol est un mauvais primaire sur une ligne dont le corps navigue) . Le hover est la bordure qui passe au bleu, sans élévation ni décalage. La grille reste la forme du catalogue flag OFF, où une carte est l'objet et non une porte.
