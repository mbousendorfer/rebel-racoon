...
| `AC-SEG-1`…`6` — two segments as tabs above the list             | **Reinstated as a radio, not tabs.** `kind` (To review / For later) is now the **Topics radio** — the first group of the Filters panel (`AC-FILT-1`), "read one kind at a time" — so the two-lane axis is one control inside the panel rather than tabs above the list. (It passed through a flat six-state filter in between; that was withdrawn too — see §2.4.) The classification rules survive as rules about the field (`AC-ST-5`): unclassified falls to `later`, and segment membership never read a content pillar in this build. |
...
⚠️ **`AC-SEG` was withdrawn, then partly reinstated.** The two segments began as tabs
above the list — _Ready to draft_ / _Topics for later_. They were withdrawn into a flat
six-state filter, and that flat filter has since been withdrawn too: `kind` is now the
**Topics RADIO** — the first group of the Filters panel (`AC-FILT-1`), "read one kind at
a time", To review / For later. So the two-lane axis is back as a control, but a radio
_inside_ the panel rather than tabs above the list — one control, nothing that can
disagree with the filter. `AC-SEG-2` / `AC-SEG-6` still hold as rules about the FIELD:
the scan classifies, unclassified falls to `later`, and nothing a reader does moves a
Topic between kinds.

| ID        | Title                                      | Criterion                                                                                                                                                                                                                                                                                                                        |
| --------- | ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AC-ST-1` | Six states, one chip vocabulary            | Exactly six: **To review · Trending · Updated · Already used · For later · Ignored**. Each — except To review — is one DS Status pill on the card and article header, same height, same shape, same slot. This is the CHIP vocabulary; the Filters panel is a separate three-group control (`AC-FILT-1`), not one row per state. |
| `AC-ST-2` | To review shows no chip                    | **To review** carries no chip. It is the absence of an answer and the commonest value in a feed; a mark meaning "nothing has happened yet" would sit on almost every row and say nothing. It is also the filter's baseline — every lane shows its To-review Topics without a control — so it has no filter control either.       |
...
| ID           | Title                                                | Criterion                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------ | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `AC-FILT-1`  | Three filter groups                                  | Three groups, each with a one-line description: **Topics** — a RADIO on `kind` ("read one kind at a time"), To review / For later, exactly one; **Marked as** — an `.ap-select` multi-select on the answered statuses (Already used / Ignored); **Sources** — an `.ap-select` multi-select on the source. Each multi-select shows its selection as chips in the trigger: the state's own pill for Marked as, the source's provenance badge for Sources. **Trending and Updated are not in the filter** — they are card chips only.                           |
| `AC-FILT-2`  | Defaults: To review lane, Already used, every source | Defaults: the **To review** lane; **Already used** ticked in Marked as, **Ignored** not; every live source. `new` (To review) is the baseline every lane shows without a control. This reproduces the old landing — the active lane, answered-but-findable work visible, ignored and parked hidden — with one panel. Ignored means "not this one"; For later is the other lane, not a hidden state. Already used stays visible: the work exists, it is findable, and hiding it is how the same Topic gets drafted twice.                                     |
| `AC-FILT-2b` | The three gates, ANDed                               | A Topic shows when its source is ticked **and** it is in the chosen lane **and** its status passes: `new` always passes (the baseline), an answered status passes only when ticked in Marked as. ⚠️ The **ignored rule falls out for free**: a signal is never read by the filter, so an ignored-but-trending Topic stays hidden until Ignored is ticked — `AC-PICK-2b` / `AC-SIG-2` hold by construction, not by a hand-written clause. Consequence: a state can be hidden, but the signals cannot be isolated; there is no "show me only what is spiking". |
| `AC-FILT-3`  | Badge counts narrowed groups, not options            | The badge counts **groups that differ from their default**, not options ticked. Two narrowed groups shows "2". At the defaults, no badge — the app's convention, kept over the mock's persistent "3".                                                                                                                                                                                                                                                                                                                                                        |
...
| ID         | Title                                      | Criterion                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `AC-SIG-1` | Signals never replace the review state     | A Topic can carry **Trending**, **Updated**, both, or neither, whatever its review state is. Neither is ever shown as a review-state pill. ⚠️ They now render as the SAME pill species as the review states — one chip vocabulary — but they never replace one: a Topic that is Already used and Trending wears both. They are card chips only, not filter rows (`AC-FILT-1`). What was withdrawn is the words "never shown as a review-state pill", not the independence underneath them. |
| `AC-SIG-2` | A signal never overrides the status filter | In the feed, a signalled Topic still shows **under its own review state and lane**. A trending Topic that's been ignored stays hidden while Ignored is unticked. A signal never overrides the filter — because the filter never reads a signal at all: Trending / Updated are not filter fields (`AC-FILT-1`, `AC-FILT-2b`).                                                                                                                                                               |
...
| `AC-PANE-7`  | The pane's card edge meets the list's CARD edges            | The pane's top edge sits on the **same y as the list's first card** — measured delta 0. A hard white card edge against a column of hard white card edges is the line a reader reads across, and one floating above the others is the first thing they notice. The pane is inset by exactly the age separator's box (`--topic-group-offset`, derived from the label's line-height plus its gap), and the separator must stay **the only thing** between the scroller's top and its first group. ⚠️ Two things broke this in turn. The list's count row moved into the column and added 37px the offset knew nothing about → the pane's edge went 44px high. Then the offset was deleted for a shared **column-top** line: true to the pixel and invisible, because the line it shared was the quietest text on the screen while the card edges were still out. The count is a page row now, outside the split. **Verify:** `.topic-card--feed` and `.topics-view__pane` report the same `getBoundingClientRect().y`. |
| `AC-PANE-8`  | The count is a page row, and it is metadata                 | The list's total is a direct child of `.topics-view` — above the reader, not inside the list column and never inside the scrollport — so the split holds only the two readers and the pane's inset has one thing to clear. It is caption weight in light ink: apparatus. ⚠️ It wore `--sys-text-style-h4-*` in `--sys-text-color-default`, the exact type `.topic-card__headline` carries below it, so the number describing the list competed with the content it describes. It stays **unfiltered** (it describes the feed, not the selection) and is absent while scanning and on both dead ends, where there is nothing to count.                                                                                                                                                                                                                                                                                                                                                                               |
...
| `AC-TITLE-2` | Every surface shows it in the same order                    | The card and the article header render one identity in one order and in one **place**: **the provenance run (source · age · state chips) on top, then the claim, then the actions**. The card's order is the one that wins — it is the shape a reader meets first and meets twelve times over, and its meta run is the editorial kicker: the small line saying where you are before the headline says what it is. ⚠️ The article led with its title until `6777c2ad`, so the door and the room presented one object two ways round; that commit aligned the CARD to the article instead and was reversed. **Verify:** a feed card's first line and the pane header's first row say the same thing in the same order.                                                                                                                                                                                                                                                                                                |
