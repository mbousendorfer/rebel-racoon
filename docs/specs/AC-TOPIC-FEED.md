...
| `AC-PANE-7`  | The two columns share their top line, structurally          | The list column and the pane start on the **same y**, and it is a consequence of the DOM — both are children of one flex row and neither offsets itself. The list's count is that column's **fixed** header, outside the scrollport, the way the pane's header is outside its body: it does not scroll away, and the two columns have one anatomy. Their first lines of text land on the same box (measured 142..156 both sides). ⚠️ It was an arithmetic inset — `margin-top: var(--topic-group-offset)`, the age separator's 20px, so the pane's edge would meet the FIRST CARD's edge. Then the count row (28px) and the scroller's own 16px gap landed above the groups: the real distance became 64px, the offset stayed 20, and the pane's top edge floated 44px above the first card and level with nothing. An offset derived from one element cannot survive a second being inserted above it, and nothing warns you. **Verify:** `.topics-view__list-col` and `.topics-view__pane` report the same `getBoundingClientRect().y`, in both the open, blank and scanning states. |
| `AC-PANE-8`  | The count is metadata, not a heading                        | The list's total is caption weight in light ink — apparatus. ⚠️ It wore `--sys-text-style-h4-*` in `--sys-text-color-default`, the exact type `.topic-card__headline` carries two rows below it, so the number describing the list competed with the content it describes and the column's loudest line was the one carrying the least. It stays **unfiltered** (it describes the feed, not the selection), and it is absent while scanning — a total posted beside five ghost cards is a number about content that is not drawn yet.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
