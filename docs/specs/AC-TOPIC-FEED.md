...
| `AC-PANE-3` — the article stacks on a narrow window              | Still true, but the LIST SHRINKS FIRST, so side by side survives a 14" laptop. When it does stack, opening an article scrolls it into view — the original left it below the fold with no feedback at all.                      |
...
| `AC-PANE-3`  | The list shrinks before the article stacks   | Both panes have a floor and share the space between them, so side by side survives a laptop-width window — **verify at 1440px: the article must be beside the list.** Only below the sum of the two floors does it stack, and the switch is measured on the space available to the two, never on the browser window. |
