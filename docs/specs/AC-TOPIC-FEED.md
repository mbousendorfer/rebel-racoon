...
| `AC-POSTS-1` | The evidence cap follows the kind          | On a `ready` Topic the posts support an argument, so the first two show and the rest sit behind an inline expander. On a `later` Topic they ARE the material and are never capped. **The section counter always states the total, capped or not.**                                                 |
