...
| `AC-PANE-3`  | The list has a width; the article takes the rest | The list is the FIXED half — a preferred width with a floor and a ceiling — and the article takes whatever is left, so a wider window buys reading room rather than a wider queue. **Verify at 1440px: the article is beside the list. Verify at 1920px: the reader is capped and centred, and the width the article gained did not go to the list.** It stacks only below the sum of the two floors, and the switch is measured on the space available to the two, never on the browser window. |
