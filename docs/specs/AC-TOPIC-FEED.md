...
| ID           | Title                                                       | Criterion                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------ | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AC-FILT-1`  | Two filter groups                                           | Two groups, in this order: **Topic status** — an `.ap-select` multi-select behind one trigger, six options, each with the count of Topics carrying it — and **Sources**, a list of checkboxes. The state counts are unfiltered: they describe the feed, so they stay still while the reader ticks, and they are where the two segments' counts went. The trigger names the **exclusions** ("All except For later, Ignored"), never a bare count: a collapsed control otherwise costs the one thing a narrowed filter has to say — what am I not seeing? |
...
| `AC-POSTS-1` | The evidence is a collapsible section, and its resting state follows the kind | **Contributing posts** is one collapsible section: the heading, the total count and a chevron on one clickable line. On a `ready` Topic it rests **closed** — the posts support an argument the reader has just finished. On a `later` Topic it rests **open** — there is no draftable angle yet and the posts ARE the material, so closing them would empty the panel. Opening it shows **every** post: the old two-visible cap and its "N more posts" link are gone, because a collapsible section plus that link is two nested disclosures for one list. The count still states the total, so folding defers the evidence rather than hiding it. |
